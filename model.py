...
class ViViTBackbone(nn.Module):
    ...
    def __init__(self, t, h, w, patch_t, patch_h, patch_w, num_classes, dim, depth, heads, mlp_dim, dim_head=3,
        ...
        self.pos_embedding = nn.Parameter(torch.randn(1, 1, self.nh * self.nw, dim)).repeat(1, self.nt, 1, 1).to('cuda:3')
...
frame = torch.randn(1, 3, 64, 128, 128).to('cuda:3')

model = MultiTaskModel().to('cuda:3')
model.count_parameters()
output1, output2, HR, RR = model(frame)
flops, params = profile(model, inputs=(frame,))

print("计算量 FLOPs: %.2f GFLOPs" % (flops / (1024 ** 3)))
